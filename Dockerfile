...
RUN curl -LO https://files.mida.sprai.org/browser/chromium_87_cov.deb
RUN apt-get update && apt-get install -y ./chromium_87_cov.deb
RUN rm chromium_87_cov.deb
...
RUN cd /usr/bin && { curl -O https://files.mida.sprai.org/browser/chrome_87_cov_unstripped ; cd -; }
